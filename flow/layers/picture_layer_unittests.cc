...
TEST_F(PictureLayerTest, InvalidPicture) {
...
TEST_F(PictureLayerTest, PaintBeforePrerollInvalidPicture) {
  const SkPoint layer_offset = SkPoint::Make(0.0f, 0.0f);
  auto layer = std::make_shared<PictureLayer>(
      layer_offset, SkiaGPUObject<SkPicture>(), false, false);
...
TEST_F(PictureLayerTest, PaintBeforePreroll) {
  const SkPoint layer_offset = SkPoint::Make(0.0f, 0.0f);
...
TEST_F(PictureLayerTest, EmptyLayer) {
  const SkPoint layer_offset = SkPoint::Make(0.0f, 0.0f);
...
#endif
           MockCanvas::DrawCall{
               1, MockCanvas::DrawPictureData{mock_picture->serialize(),
                                              SkPaint(), SkMatrix()}},
